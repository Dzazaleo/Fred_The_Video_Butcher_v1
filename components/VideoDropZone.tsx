...
export const VideoDropZone: React.FC<VideoDropZoneProps> = ({ onFileSelected }) => {
  ...
  const validateFile = (file: File): boolean => {
    ...
    // Some browsers might report empty type for some containers, checking extension as fallback
  ...
  const getBorderColor = () => {
    if (error) return 'border-red-400 bg-red-50/50';
    if (isDragging) return 'border-blue-500 bg-blue-50/50';
    return 'border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50/50';
  };
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      >
        ...
        <div className={`
          ...
          ${error ? 'bg-red-100 text-red-600' : isDragging ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500 group-hover:bg-slate-200 group-hover:text-slate-600'}
        `}>
          {error ? <AlertCircle size={32} /> : <Upload size={32} />}
        </div>

        <div className="text-center space-y-2">
          <h3 className={`text-lg font-semibold ${error ? 'text-red-700' : 'text-slate-800'}`}>
